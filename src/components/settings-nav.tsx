...
export function SettingsNav({ active }: { active: SettingsSection }) {
  return (
    <aside className="lg:sticky lg:top-20 lg:self-start">
      ...
      <nav aria-label="설정 메뉴" className="grid grid-cols-2 border-y border-finance-hairline lg:block lg:border-y-0 lg:border-l lg:border-finance-hairline">
        {ITEMS.map((item) => (
          <Link
            aria-current={active === item.key ? 'page' : undefined}
            className={`block border-b-2 px-4 py-3 text-[13px] font-semibold lg:border-b-0 lg:border-l-2 ${active === item.key ? 'border-finance-blue bg-finance-blue-tint text-finance-blue lg:-ml-px' : 'border-transparent text-finance-muted hover:text-finance-ink'}`}
            href={item.href}
            key={item.key}
          >
            {item.label}

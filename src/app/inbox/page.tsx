  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="mx-auto max-w-[1680px] px-5 pb-14 pt-9 sm:px-12">
        <header>
          <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-finance-blue">새 거래 들여오기</p>
          <h1 className="mt-2 text-[30px] font-bold leading-none tracking-[-0.03em] text-finance-ink">가져오기</h1>
          <p className="mt-2 text-xs text-finance-muted">파일을 올리고 추천 결과를 확인한 뒤, 안전하게 거래 원장으로 반영합니다.</p>
        </header>
        ...
        <nav aria-label="가져오기 작업" className="mt-6 flex overflow-x-auto border-b border-finance-border">
          {tabs.map((item) => (
            <Link
              ...
              className={`shrink-0 border-b-2 px-4 py-3 text-xs font-semibold ${tab === item.key ? 'border-finance-blue text-finance-blue' : 'border-transparent text-finance-muted hover:text-finance-ink'}`}
              href={`/inbox?tab=${item.key}`}
              key={item.key}
            >
              {item.label}{item.count !== undefined && <span className="ml-1.5 bg-finance-track px-2 py-0.5 text-[10px] text-finance-muted">{item.count.toLocaleString('ko-KR')}</span>}
            </Link>
          ))}
        </nav>
        ...
        {tab === 'review' && (
          <section className="mt-6">
            <div className="mb-3 border-t border-finance-ink pt-4">
              ...
              <p className="mt-1 text-xs text-finance-muted">자동 분류를 포함한 모든 거래를 수정할 수 있습니다. 한 건씩 반영하거나 그룹을 선택해 한 번에 처리하세요.</p>
            </div>
            {data.truncated && <p className="mb-3 border-l-2 border-finance-amber py-2 pl-3 text-[13px] text-finance-muted">대기 거래가 많아 최근 500건만 표시합니다. 먼저 반영하거나 제외하면 나머지가 이어서 표시됩니다.</p>}
            {data.items.length > 0 ? (
              ...
              <div className="border-y border-dashed border-finance-border px-6 py-14 text-center">
                ...
                <Link className="mt-3 inline-flex text-xs font-semibold text-finance-blue" href="/inbox?tab=upload">새 파일 업로드 →</Link>
              </div>
            )}
          </section>
        )}
        ...
        {tab === 'upload' && (
          <section className="mt-6 border-b border-finance-border border-t border-finance-ink py-5">
            ...
            <p className="mt-1 text-xs leading-5 text-finance-muted">뱅크샐러드 파일은 DJ·YJ 최대 2개를, 카드사 명세서는 카드사와 소유자를 선택해 올려 주세요.</p>
        ...
        {tab === 'history' && (
          <section className="mt-6 border-t border-finance-ink">
            <div className="border-b border-finance-border py-4">
              <h2 className="text-sm font-bold text-finance-ink">처리 기록</h2>
              <p className="mt-1 text-xs text-finance-muted">같은 날 같은 소스로 처리한 행은 하나의 기록으로 묶입니다. 같은 소스를 하루에 여러 번 올리면 합쳐 보이는 1차 기록입니다.</p>
            </div>
            {data.history.length > 0 ? (
              <div className="divide-y divide-finance-track">
                {data.history.map((entry) => (
                  <article className="grid gap-3 py-4 text-[13px] sm:grid-cols-[140px_minmax(180px,1fr)_auto_auto_auto] sm:items-center" key={`${entry.source}:${entry.processedOn}`}>
                    <time className="text-finance-muted">{entry.processedOn}</time>
                    <div><strong className="text-finance-ink">{entry.label}</strong><p className="mt-1 text-[11px] text-finance-faint">거래 기간 {entry.earliestMonth}~{entry.latestMonth}</p></div>
                    ...
                    <Link className="text-xs font-semibold text-finance-blue" href={`/ledger?month=${entry.latestMonth}&tab=list`}>거래 보기 →</Link>
                  </article>
                ))}
              </div>
            ) : (
              <div className="border-b border-dashed border-finance-border py-14 text-center"><p className="font-medium text-finance-ink">아직 처리 기록이 없습니다.</p><Link className="mt-3 inline-flex text-xs font-semibold text-finance-blue" href="/inbox?tab=upload">첫 파일 업로드 →</Link></div>
            )}
          </section>
        )}
        ...
        {tab === 'unclassified' && unclassifiedData && (
          <section className="mt-6 space-y-3">
            ...
            {unclassifiedData.counts.unclassified > unclassifiedData.unclassified.length && <p className="text-center text-xs text-finance-muted">최근 100개만 표시됩니다. 먼저 보이는 거래를 분류하면 다음 항목이 나타납니다.</p>}
          </section>
        )}
      </main>
    </div>

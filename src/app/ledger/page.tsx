    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="mx-auto max-w-[1680px] px-5 pb-14 pt-9 sm:px-12">
        <header className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-finance-blue">월간 기록과 분석</p>
            <h1 className="mt-2 text-[30px] font-bold leading-none tracking-[-0.03em] text-finance-ink">거래</h1>
            <p className="mt-2 text-xs text-finance-muted">필터를 한 번 잡고 합계에서 거래 행까지 내려봅니다</p>
          </div>
          <div className="flex flex-wrap items-center justify-end gap-2">
            {tab !== 'list' && <Link className="h-[34px] bg-finance-blue px-4 py-2 text-xs font-semibold text-white hover:opacity-80" href={`${ledgerUrl(shell.month, filters, { tab: 'list' })}#transaction-form`}>거래 추가</Link>}
            <div className="flex items-center border border-finance-ink">
              <Link aria-label="이전 달" className="grid h-8 w-[34px] place-items-center border-r border-finance-ink text-[13px] hover:bg-finance-track" href={ledgerUrl(shell.previousMonth, filters, { tab })}>←</Link>
              <form action="/ledger" className="flex h-8 items-center">
                ...
                <input aria-label="조회 월" className="h-8 w-[124px] border-0 bg-white px-2 text-center text-[13px] font-semibold text-finance-ink outline-none" defaultValue={shell.month} max={shell.latestMonth} name="month" type="month" />
              ...
              <Link aria-label="다음 달" className="grid h-8 w-[34px] place-items-center border-l border-finance-ink text-[13px] hover:bg-finance-track" href={ledgerUrl(shell.nextMonth, filters, { tab })}>→</Link>
            </div>
          </div>
        </header>
        ...
        <section className="flex flex-col gap-3 border-b border-finance-border py-4 sm:flex-row sm:items-center">
          ...
          <div className="ml-auto flex items-center gap-3">
            <Link className="text-xs font-semibold text-finance-muted hover:text-finance-blue" href="/settings?section=recurring">규칙 설정</Link>
            {recurringPending > 0 && (
        ...
        {tab === 'list' && listData && (
          <>
            ...
            <section className="mt-6">
              <div className="flex items-baseline justify-between border-t border-finance-ink pt-4"><h2 className="text-sm font-bold text-finance-ink">거래 내역</h2><span className="text-[11px] text-finance-faint">최근순</span></div>

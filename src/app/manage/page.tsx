...
const secondaryButton = 'h-[34px] border border-finance-hairline bg-white px-3 t-body-strong text-finance-ink hover:bg-finance-panel'
const saveButton = 'h-[34px] bg-finance-ink px-3 t-body-strong text-white hover:opacity-80'
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="mx-auto w-full max-w-[1440px] px-5 pb-14 pt-10 sm:px-12">
        ...
        <div>
          <p className="t-label uppercase text-finance-blue">기준 정보</p>
          <h1 className="mt-2 t-page-title text-finance-ink">가계부 관리</h1>
          <p className="mt-2 t-caption text-finance-muted">결제수단과 카테고리, 가맹점별 추천 기준을 한곳에서 관리합니다.</p>
        </div>
          ...
          <section className="mt-6 space-y-8">
            <div className="border-t border-finance-ink py-4">
              <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
                <div><h2 className="t-section text-finance-ink">가맹점 사전</h2><p className="mt-1 t-caption text-finance-muted">총 {data.counts.rules.toLocaleString('ko-KR')}개 · 사용자 확정과 AI 추천을 사용 횟수 순으로 100개까지 표시</p></div>
            ...
            <div>
              <h2 className="border-t border-finance-ink pt-4 t-section text-finance-ink">뱅크샐러드 결제수단 별칭</h2>
              <p className="mt-1 t-caption text-finance-muted">업로드 파일의 결제수단 이름을 우리 가계부 계정에 연결합니다. 새 별칭은 인박스 반영 과정에서 자동 학습됩니다.</p>
              <div className="mt-4 grid gap-3 lg:grid-cols-2">
                {data.aliases.map((alias) => (
                  <form action={saveAlias} className="grid items-end gap-2 border-b border-finance-hairline py-4 sm:grid-cols-[.5fr_1fr_1.2fr_auto_auto]" key={`${alias.owner}:${alias.alias}`}>
                    ...
                    <div><p className="t-caption text-finance-muted">소유자</p><p className="mt-2 t-body font-medium text-finance-ink">{alias.owner}</p></div>
                    <div className="min-w-0"><p className="t-caption text-finance-muted">파일 표기</p><p className="mt-2 truncate t-body font-medium text-finance-ink" title={alias.alias}>{alias.alias}</p></div>
                    <label className="grid gap-1 t-caption font-medium text-finance-muted">연결<select className={inputClass} defaultValue={alias.accountId} name="accountId">{data.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label>
                    <SubmitButton className={saveButton} pendingLabel="저장 중…" type="submit">저장</SubmitButton>
                    <SubmitButton className="rounded-lg px-2 py-2 t-body font-medium text-finance-red hover:bg-finance-red-tint disabled:opacity-60" name="intent" pendingLabel="삭제 중…" type="submit" value="delete">삭제</SubmitButton>
                  </form>
                ))}
              </div>
            </div>

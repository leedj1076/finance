...
function BudgetBullet({
  ...
  return (
    <div className="grid items-center gap-2 text-xs sm:grid-cols-[110px_minmax(180px,1fr)_180px_100px] sm:gap-5">
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="mx-auto max-w-[1440px] px-5 pb-14 pt-9 sm:px-12">
        <header>
          <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-finance-blue">이번 달</p>
        ...
        <section className="mt-6 grid border-y border-finance-ink lg:grid-cols-3 lg:divide-x lg:divide-finance-border">
          <article className="flex min-h-[202px] items-center gap-6 border-b border-finance-border py-6 lg:border-b-0 lg:pr-8">
            ...
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.09em] text-finance-muted">목표대로 가고 있나</p>
              <h2 className="mt-2 text-lg font-bold text-finance-ink">이번 달 순저축률</h2>
              <p className="mt-2 text-xs leading-5 text-finance-muted">
          ...
          <article className="min-h-[202px] border-b border-finance-border py-7 lg:border-b-0 lg:px-8">
            <p className="text-[11px] font-semibold uppercase tracking-[0.09em] text-finance-muted">이번 달 더 써도 되나</p>
            {data.safeToSpend?.hasIncome ? (
              <>
                <p className={`mt-4 text-[34px] font-bold leading-none tracking-[-0.04em] ${data.safeToSpend.remaining >= 0 ? 'text-finance-green' : 'text-finance-red'}`}>
                  {data.safeToSpend.remaining < 0 && '−'}{formatWon(Math.abs(data.safeToSpend.remaining))}<span className="ml-1 text-base font-medium text-finance-muted">원</span>
                </p>
              ...
              <div className="mt-7 border-l-2 border-finance-faint pl-4">
                <p className="text-sm font-semibold text-finance-ink">수입 기록이 필요합니다</p>
                <p className="mt-2 text-xs leading-5 text-finance-muted">완료된 달의 수입이 쌓이면 저축 목표 기반 지출 상한을 계산합니다.</p>
              </div>
            )}
          </article>

          <Link className="block min-h-[202px] py-7 lg:pl-8" href="/assets">
            <p className="text-[11px] font-semibold uppercase tracking-[0.09em] text-finance-muted">자산이 늘고 있나</p>
            {latestNetWorth ? (
              <>
                <p className="mt-4 text-[30px] font-bold leading-none tracking-[-0.04em] text-finance-ink">{formatWon(latestNetWorth.netWorth)}<span className="ml-1 text-base font-medium text-finance-muted">원</span></p>
                <div className="mt-5 flex items-center justify-between gap-5">
                  <p className={`text-xs font-semibold ${netWorthDelta === null ? 'text-finance-muted' : netWorthDelta >= 0 ? 'text-finance-green' : 'text-finance-red'}`}>
                ...
                <p className="mt-3 text-[11px] text-finance-faint">총자산 {formatWon(latestNetWorth.assets)}원 · 부채 {formatWon(latestNetWorth.liabilities)}원</p>
              </>
            ) : (
              <div className="mt-7 border-l-2 border-finance-faint pl-4">
                <p className="text-sm font-semibold text-finance-ink">자산을 입력하면 추이가 보입니다</p>
                <p className="mt-2 text-xs text-finance-muted">자산 화면에서 잔고를 보정할 수 있습니다. →</p>
              </div>
            )}
          </Link>
        </section>
        ...
        <section aria-label="해야 할 일" className="border-b border-finance-border">
          {visibleTodos.length > 0 ? visibleTodos.map((todo) => (
            <Link className="flex min-h-12 flex-col gap-1 border-b border-finance-track py-3 text-[13px] last:border-b-0 sm:flex-row sm:items-center sm:gap-3" href={todo.href} key={todo.kind}>
              <span aria-hidden className={`h-[7px] w-[7px] shrink-0 ${TODO_TONES[todo.kind]}`} />
              <strong className="text-finance-ink">{todo.title}</strong>
              <span className="text-xs text-finance-muted">{todo.detail}</span>
              <span className="ml-auto shrink-0 text-xs font-semibold text-finance-blue">확인 →</span>
            </Link>
          )) : (
            <p className="flex min-h-12 items-center gap-3 py-3 text-[13px] text-finance-muted"><span aria-hidden className="h-[7px] w-[7px] bg-finance-green" />지금 바로 확인할 일은 없습니다.</p>
          )}
        </section>
        ...
        <section className="grid gap-10 border-b border-finance-border py-7 xl:grid-cols-[minmax(0,1.35fr)_minmax(440px,0.8fr)]">
          <article className="min-w-0">
            <div className="flex items-baseline justify-between">
              <div><h2 className="text-sm font-bold text-finance-ink">순자산 추이</h2><p className="mt-1 text-[11px] text-finance-faint">최근 12개월 · 잔액이 없는 달은 직전 값 유지</p></div>
              <Link className="text-xs font-semibold text-finance-blue" href="/assets">자산 보기 →</Link>
            </div>
            {netWorth.length > 0 ? (
              <div className="mt-5 overflow-x-auto"><NetWorthChart data={netWorth.map((point) => ({ ...point, debt: point.liabilities, active: true }))} /></div>
            ) : (
              <p className="mt-5 grid min-h-[260px] place-items-center border-y border-finance-border text-[13px] text-finance-muted">자산을 입력하면 12개월 추이가 보입니다.</p>
            )}
          </article>
          <article className="min-w-0 xl:border-l xl:border-finance-border xl:pl-10">
            <div><h2 className="text-sm font-bold text-finance-ink">이번 달 돈의 흐름</h2><p className="mt-1 text-[11px] text-finance-faint">수입에서 지출과 저축 납입을 차례로 차감</p></div>
            {data.current.income + data.current.expense + data.current.saving > 0 ? (
              ...
              <p className="mt-5 grid min-h-[220px] place-items-center border-y border-finance-border text-[13px] text-finance-muted">이번 달 거래를 입력하면 돈의 흐름이 보입니다.</p>
            )}
          </article>
        </section>
        ...
        <section className="border-b border-finance-border py-7">
          <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
            <div><h2 className="text-sm font-bold text-finance-ink">예산 대비 지출</h2><p className="mt-1 text-[11px] text-finance-faint">막대 = 실제 · 세로선 = 예산 · 옅은 구간 = 오늘까지 적정 페이스({Math.round(data.pace.percent)}%)</p></div>
            <p className="text-xs text-finance-muted">대분류 예산 합계 <strong className="text-finance-ink">{formatWon(data.budget.total)}원</strong> 중 <strong className="text-finance-ink">{formatWon(data.budget.actual)}원</strong> 사용 · <strong>{data.budget.percent === null ? '예산 미설정' : `${formatRate(data.budget.percent)}%`}</strong></p>
          </div>
          {budgetRows.length > 0 ? (
            ...
            <p className="mt-5 grid min-h-24 place-items-center border-y border-finance-border text-[13px] text-finance-muted">이번 달 예산이나 지출이 없습니다.</p>
          )}
        </section>

        <section className="border-b border-finance-border py-7">
          <div className="flex items-baseline justify-between gap-4">
            <div><h2 className="text-sm font-bold text-finance-ink">카테고리별 추세</h2><p className="mt-1 text-[11px] text-finance-faint">최근 6개월 · 마지막 점이 이번 달</p></div>
            <Link className="text-xs font-semibold text-finance-blue" href={`/report?year=${year}`}>연간 › 항목별 월별 표 →</Link>
          </div>
          {categoryRows.length > 0 ? (
            ...
            <p className="mt-5 grid min-h-24 place-items-center border-y border-finance-border text-[13px] text-finance-muted">이번 달 카테고리 지출이 없습니다.</p>
          )}
        </section>

        <section className="grid gap-10 py-7 xl:grid-cols-[minmax(0,1.35fr)_minmax(420px,0.8fr)]">
          <article className="min-w-0">
            <div className="flex items-baseline justify-between"><div><h2 className="text-sm font-bold text-finance-ink">월별 수입 · 지출</h2><p className="mt-1 text-[11px] text-finance-faint">{year}년 1~{Number(month.slice(5))}월 · 다른 해는 연간에서</p></div><Link className="text-xs font-semibold text-finance-blue" href="/report">연간 →</Link></div>
            <div className="mt-5 overflow-x-auto"><MonthlyCashflowChart data={data.monthly} /></div>
            <p className="mt-2 text-[11px] text-finance-muted">올해 누적 · 수입 <strong className="text-finance-blue">{formatWon(data.annual.income)}</strong> · 지출 <strong className="text-finance-ink">{formatWon(data.annual.expense)}</strong> · 순저축 <strong className="text-finance-green">{formatWon(data.annual.netSaving)}</strong></p>
          </article>
          <article className="min-w-0 xl:border-l xl:border-finance-border xl:pl-10">
            <div><h2 className="text-sm font-bold text-finance-ink">월별 저축률</h2><p className="mt-1 text-[11px] text-finance-faint">점선 = 목표 {formatRate(data.savingsTarget)}% · 연 누적 {formatRate(data.annual.savingsRate)}%</p></div>
            <div className="mt-4 overflow-x-auto"><SavingsRateChart data={data.monthly} target={data.savingsTarget} /></div>
            <p className="mt-2 text-[11px] text-finance-muted">목표 달성 <strong className="text-finance-green">{targetReached.length}개월</strong>{bestMonth && <> · 최고 {Number(bestMonth.month.slice(5))}월 {formatRate(bestMonth.savingsRate)}%</>}{worstMonth && <> · 최저 {Number(worstMonth.month.slice(5))}월 {formatRate(worstMonth.savingsRate)}%</>}</p>
          </article>
        </section>
      </main>
    </div>

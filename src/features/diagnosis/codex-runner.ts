...
import type { DiagnosisErrorCode, DiagnosisReport, DiagnosisSnapshot } from './types'

export type CodexDiagnosisOptions = {
  codexPath: string
  model?: string
  signal?: AbortSignal
  timeoutMs?: number
...
// These are operator-owned options, never values taken from the queued snapshot.
...
function killGroup(pid: number | undefined, signal: NodeJS.Signals) {
  if (!pid) return
  try {
    // detached:true gives this invocation its own process group on macOS/Linux.
    process.kill(process.platform === 'win32' ? pid : -pid, signal)
  } catch {
    // An already-exited invocation has no group left to stop.
  }
}

async function readFinalReport(path: string, snapshot: DiagnosisSnapshot): Promise<DiagnosisReport> {
  try {
    ...
    try {
      ...
      return parseDiagnosisReport(JSON.parse(buffer.subarray(0, bytesRead).toString('utf8')), snapshot)
    } finally {
      await file.close()
    }
  } catch {
...
export async function runCodexDiagnosis(snapshot: DiagnosisSnapshot, options: CodexDiagnosisOptions): Promise<DiagnosisReport> {
  if (options.signal?.aborted) throw new DiagnosisRunnerError('worker_stopped')
  if (!isAbsolute(options.codexPath) || (options.model !== undefined && !/^[a-zA-Z0-9][a-zA-Z0-9._-]{0,99}$/.test(options.model))) {
  ...
  try {
    ...
    await writeFile(schemaPath, JSON.stringify(DIAGNOSIS_REPORT_SCHEMA), { mode: 0o400, flag: 'wx' })
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const consumeEvent = (line: string) => {
        ...
        try {
          ...
          if (event.type === 'turn.failed' || event.type === 'error') {
            ...
            // This worker expects text only. Reject tools and unfamiliar item kinds.
      ...
      child.stderr.on('data', (chunk: Buffer) => {
        // Drain stderr without storing or logging private CLI diagnostics.
    ...
    if (options.signal?.aborted) throw new DiagnosisRunnerError('worker_stopped')
    return await readFinalReport(outputPath, snapshot)
  } catch (error) {

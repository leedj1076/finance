  ...
  return (
    <form action={bulkSaveCategories} className="mt-6">
      ...
      <div className="sticky top-14 z-20 flex flex-col gap-3 border-y border-finance-ink bg-white/95 py-3 backdrop-blur sm:flex-row sm:items-center sm:justify-between">
        <div><h2 className="t-section text-finance-ink">카테고리 구조</h2><p className="mt-1 t-caption text-finance-muted">대분류와 소분류를 같은 화면에서 편집하고 끌어서 순서를 바꿉니다.</p></div>
        <SaveButton />
      </div>
      {(['expense', 'income', 'saving'] as const).map((kind) => {
        ...
        return (
          <section className="mt-6" key={kind}>
            <div className="mb-3 flex items-center justify-between"><h3 className="t-section text-finance-ink">{labels[kind]} <span className="t-caption font-normal text-finance-faint">{kindGroups.reduce((sum, group) => sum + group.rows.length, 0)}개</span></h3><button className="h-[30px] border border-finance-hairline bg-white px-3 t-body-strong text-finance-ink hover:bg-finance-panel" onClick={() => addGroup(kind)} type="button">+ 대분류 추가</button></div>
                ...
                <article className={`overflow-hidden border border-finance-hairline border-l-4 bg-white ${tones[kind]} ${draggingGroup === group.key ? 'opacity-40' : ''}`} key={group.key} onDragOver={(event) => event.preventDefault()} onDrop={() => dropGroup(group)}>
                  ...
                  <div className="divide-y divide-finance-hairline">
                    {group.rows.map((row, index) => (
                      <div className={`grid grid-cols-[28px_minmax(0,1fr)_auto_auto_auto] items-center gap-2 px-3 py-2 ${row.deleted ? 'bg-finance-red-tint opacity-60' : ''}`} key={row.key} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.stopPropagation(); dropSub(group.key, row.key) }}>
                        <span className="cursor-grab text-center text-finance-faint" draggable onDragEnd={() => setDraggingSub(null)} onDragStart={(event) => { event.stopPropagation(); setDraggingSub({ group: group.key, row: row.key }) }}>⠿</span>
                        ...
                        <span className="min-w-16 text-right t-caption text-finance-faint">사용 {row.usage.toLocaleString('ko-KR')}건</span>
                        <label className="flex items-center gap-1.5 whitespace-nowrap t-caption text-finance-muted"><input checked={row.hidden} onChange={(event) => updateGroup(group.key, (current) => ({ ...current, rows: current.rows.map((candidate) => candidate.key === row.key ? { ...candidate, hidden: event.target.checked } : candidate) }))} type="checkbox" /> 숨김</label>
                        <button aria-label={`${row.sub || '새 소분류'} ${row.deleted ? '삭제 취소' : '삭제'}`} className={`rounded px-2 py-1 t-body ${row.deleted ? 'text-finance-red' : 'text-finance-faint hover:bg-finance-red-tint hover:text-finance-red'}`} onClick={() => removeSub(group.key, row)} type="button">{row.deleted ? '↶' : '×'}</button>
                      </div>
                    ))}
                  </div>
                  <button className="m-2 rounded-md px-2 py-1.5 t-body-strong text-finance-blue hover:bg-finance-blue-tint" onClick={() => addSub(group.key)} type="button">+ 소분류 추가</button>
                </article>
              ))}
            </div>
          </section>
      ...
      <p className="mt-5 border-l-2 border-finance-amber bg-finance-amber-tint px-4 py-3 t-caption text-finance-amber">사용 이력이 없는 항목은 삭제되고, 거래나 정기거래가 연결된 항목은 기록 보존을 위해 숨김 처리됩니다.</p>
    </form>
  )
}

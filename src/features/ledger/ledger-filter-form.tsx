...
export function LedgerFilterForm({ accounts, filters, majorOptions, month }: LedgerFilterFormProps) {
  ...
  return (
    <form action="/ledger" className="flex flex-wrap items-center gap-2 border-b border-finance-border py-4" ref={formRef}>
      ...
      {anyFilter && <Link className="self-center text-center text-xs font-semibold text-finance-blue hover:text-finance-ink" href={ledgerUrl(month, { account: '', flow: '', major: '', q: '' })}>초기화</Link>}
    </form>
  )
}

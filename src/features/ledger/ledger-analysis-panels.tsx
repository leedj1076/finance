...
function SummaryCard({ label, value, note, tone = 'ink' }: {
  ...
  return (
    <article className="px-4 py-5 first:pl-0 last:pr-0 sm:px-6">
      ...
      <p className={`mt-2 text-[24px] font-semibold leading-none tabular-nums ${toneClass}`}>{value}</p>
    ...
    <div className="mt-6">
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-2">
        <article className="border-t border-finance-ink pt-4">
          <h2 className="text-sm font-bold text-finance-ink">가장 많이 변한 카테고리</h2>
          <div className="mt-3 divide-y divide-finance-hairline">
            {[...data.ranks]
              .sort((left, right) => Math.abs(right.delta) - Math.abs(left.delta))
              .slice(0, 6)
              .map((rank) => (
                <div className="flex items-center justify-between gap-4 py-3 text-sm" key={rank.major}>
            ...
            {data.ranks.length === 0 && <p className="py-10 text-center text-sm text-finance-muted">비교할 거래가 없습니다.</p>}
          </div>
        </article>
        <article className="border-t border-finance-ink pt-4">
          <h2 className="text-sm font-bold text-finance-ink">큰 거래 TOP 10</h2>
          <div className="mt-3 divide-y divide-finance-hairline">
            {data.topTransactions.map((transaction, index) => (
              <div className="flex items-center gap-3 py-3" key={transaction.id}>
                <span className="w-5 text-xs text-finance-faint">{index + 1}</span>
                <div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-finance-ink">{transaction.merchant || transaction.major}</p><p className="mt-0.5 text-xs text-finance-muted">{transaction.date} · {transaction.major}{transaction.accountName ? ` · ${transaction.accountName}` : ''}</p></div>
                <span className="shrink-0 text-sm font-semibold tabular-nums text-finance-ink">{formatWon(transaction.amount)}원</span>
              </div>
            ))}
            {data.topTransactions.length === 0 && <p className="py-10 text-center text-sm text-finance-muted">거래가 없습니다.</p>}
          </div>
        </article>
      </section>
    </div>
  ...
  return (
    <div className="mt-6 grid gap-8 xl:grid-cols-[minmax(300px,0.7fr)_minmax(0,1.3fr)]">
      <section className="border-t border-finance-ink pt-4">
        <h2 className="text-sm font-bold text-finance-ink">카테고리 순위</h2>
        <p className="mt-1 text-xs text-finance-muted">카테고리를 선택하면 이 화면에서 소분류와 가맹점을 펼칩니다.</p>
        <div className="mt-5 space-y-4">
          {data.ranks.map((rank, index) => (
            <div key={rank.major}>
              <div className="grid grid-cols-[24px_minmax(90px,1fr)_auto] items-center gap-2 text-sm">
                ...
                <div className="text-right"><p className="font-semibold tabular-nums text-finance-ink">{formatWon(rank.amount)}원</p><p className={`mt-0.5 text-[11px] ${rank.delta > 0 ? 'text-finance-red' : rank.delta < 0 ? 'text-finance-green' : 'text-finance-faint'}`}>{rank.delta === 0 ? '–' : `${rank.delta > 0 ? '▲' : '▼'} ${formatWon(Math.abs(rank.delta))}원`}</p></div>
              </div>
          ...
          {data.ranks.length === 0 && <p className="py-10 text-center text-sm text-finance-muted">이 필터에는 거래가 없습니다.</p>}
        </div>
      </section>
      ...
      <section className="border-t border-finance-ink pt-4">
        {!detail || !filters.major ? (
          <div className="grid min-h-56 place-items-center border-y border-finance-hairline text-center">
            <div><p className="text-sm font-semibold text-finance-ink">왼쪽에서 카테고리를 선택하세요</p><p className="mt-1 text-xs text-finance-muted">소분류·가맹점 구성과 해당 거래를 한 번에 확인할 수 있습니다.</p></div>
          </div>
        ) : (
          <>
            <div className="flex flex-wrap items-start justify-between gap-3 border-b border-finance-hairline pb-4">
              <div><p className="text-xs text-finance-muted">선택 카테고리</p><h2 className="mt-1 text-xl font-bold text-finance-ink">{filters.major}</h2><p className="mt-1 text-xs text-finance-muted">{detail.transactions.length}건 · 전체 대비 {formatRate(detail.percent)}%</p></div>
              <Link className="h-[30px] bg-finance-ink px-3 py-1.5 text-xs font-semibold text-white hover:bg-finance-blue" href={ledgerUrl(data.month, filters, { tab: 'list' })}>거래 보기 →</Link>
            </div>
            <div className="grid gap-6 pt-5 md:grid-cols-2">
              <div><h3 className="text-xs font-semibold uppercase tracking-[0.08em] text-finance-muted">소분류</h3><div className="mt-2 divide-y divide-finance-hairline">{detail.subs.map((sub) => <div className="flex justify-between gap-3 py-3 text-sm" key={sub.sub}><span>{sub.sub} <small className="text-finance-faint">{sub.count}건</small></span><strong className="tabular-nums">{formatWon(sub.amount)}원</strong></div>)}</div></div>
              <div><h3 className="text-xs font-semibold uppercase tracking-[0.08em] text-finance-muted">가맹점</h3><div className="mt-2 divide-y divide-finance-hairline">{detail.merchants.map((merchant) => <div className="flex justify-between gap-3 py-3 text-sm" key={merchant.name}><span className="truncate">{merchant.name} <small className="text-finance-faint">{merchant.count}건</small></span><strong className="shrink-0 tabular-nums">{formatWon(merchant.amount)}원</strong></div>)}</div></div>
            </div>
          </>
        )}
      </section>
...
export function LedgerMerchantsPanel({ data, filters }: {
  ...
  return (
    <section className="mt-6 overflow-hidden border-t border-finance-ink">
      <div className="border-b border-finance-hairline py-4"><h2 className="text-sm font-bold text-finance-ink">가맹점별 지출</h2><p className="mt-1 text-xs text-finance-muted">가맹점을 누르면 같은 필터를 유지한 채 거래 목록으로 이동합니다.</p></div>

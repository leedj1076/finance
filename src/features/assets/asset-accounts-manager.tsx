...
function SaveButton() {
  ...
  return <button className="h-[34px] bg-finance-ink px-4 t-body-strong text-white hover:bg-finance-blue disabled:opacity-50" disabled={pending} type="submit">{pending ? '저장 중…' : '계정 변경 저장'}</button>
}
  ...
  return (
    <form action={action} className="mt-6">
      ...
      <div className="flex flex-col justify-between gap-3 border-y border-finance-ink py-4 sm:flex-row sm:items-center">
        <div><h2 className="t-section text-finance-ink">자산 계정</h2><p className="mt-1 t-caption text-finance-muted">이름·그룹·사용 여부를 관리합니다. 월별 잔액은 자산 화면의 잔고 보정에서 입력합니다.</p></div>
        <SaveButton />
      </div>

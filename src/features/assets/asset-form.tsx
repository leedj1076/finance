    ...
    <form action={action} className="mt-6">
      ...
      <section className="overflow-hidden border-t border-finance-ink">
        <div className="flex flex-col justify-between gap-4 border-b border-finance-hairline py-4 sm:flex-row sm:items-center">
          <div>
            <h2 className="t-section text-finance-ink">잔고 보정 · {month}</h2>
              ...
              <section className="bg-white p-5" key={`${group.kind}-${group.major}`}>
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <h3 className="t-section text-finance-ink">{group.major}</h3>
                    {group.kind === 'liability' && (
                      <span className="bg-finance-red-tint px-2 py-0.5 t-badge text-finance-red">부채</span>
                    )}
                  </div>
                  <span className="t-caption text-finance-muted">소계 {formatWon(group.subtotal)}원</span>
                </div>
                ...
                <div className="mt-4 space-y-2">
                  {orderedRows.map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {balanceOnly ? (
                          <><input name={`name:${row.id}`} type="hidden" value={row.name} /><span className="t-body font-medium text-finance-ink">{row.name}</span></>
                        ) : (
                          <>
                            <span className="cursor-grab text-center text-finance-faint" draggable onDragEnd={() => setDraggingId(null)} onDragStart={() => setDraggingId(row.id)} title="끌어서 순서 변경">⠿</span>
                            <input aria-label={`${row.name} 이름`} className={`h-[34px] min-w-0 border border-finance-hairline bg-white px-3 t-body text-finance-ink outline-none focus:border-finance-blue ${deleted ? 'line-through' : ''}`} defaultValue={row.name} name={`name:${row.id}`} type="text" />
                          </>
                        )}
                        <label className="relative">
                          ...
                          <input
                            ...
                            className="h-[34px] w-full border border-finance-hairline bg-white px-3 pr-7 text-right t-body tabular-nums text-finance-ink outline-none focus:border-finance-blue"
                            ...
                            type="text"
                          />
                          <span className="pointer-events-none absolute right-2 top-2.5 t-caption text-finance-faint">원</span>
                        </label>
                        {!balanceOnly && <button
                          aria-label={`${row.name} ${deleted ? '삭제 취소' : '삭제'}`}
                          className={`h-[34px] t-body-strong ${deleted ? 'bg-finance-red-tint text-finance-red' : 'text-finance-faint hover:bg-finance-track hover:text-finance-ink'}`}
                  ...
                  {!balanceOnly && groupNewRows.map((row) => (
                    <div className="grid grid-cols-[24px_minmax(0,1fr)_minmax(115px,0.8fr)_36px] items-center gap-2 border-b border-finance-green bg-finance-green-tint p-2" key={row.key}>
                      <span className="text-center text-finance-faint">＋</span>
                      <input
                        aria-label={`${group.major} 새 항목 이름`}
                        autoFocus
                        className="h-[34px] min-w-0 border border-finance-hairline bg-white px-3 t-body text-finance-ink outline-none focus:border-finance-blue"
                      ...
                      <button
                        ...
                        className="h-[34px] t-body-strong text-finance-faint hover:bg-finance-green-tint hover:text-finance-ink"
                ...
                {!balanceOnly && <button
                  className="mt-3 h-[30px] px-2 t-body-strong text-finance-blue hover:bg-finance-blue-tint"

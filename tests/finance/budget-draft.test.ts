...
test('an invalid edit stays invalid and retains recommendation provenance', () => {
  const rows = baseline(); rows[0].recommendationJobId = job
  const state = budgetDraftReducer(createBudgetDraft(rows), { type: 'edit', major: '식비', amount: '' })

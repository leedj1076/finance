...
test('family user can manage a transaction and change their password', async ({ page }, testInfo) => {
  ...
  try {
    ...
    const budgetInput = page.getByRole('spinbutton', { name: '식비 예산', exact: true })
    const editableBudgetForms = page.locator('form').filter({ has: budgetInput })
    await expect(editableBudgetForms).toHaveCount(1)
    await expect(budgetInput).toHaveCount(1)
    await expect(page.locator('table').getByRole('spinbutton', { name: '식비 예산', exact: true })).toHaveCount(0)
    ...
    await expect(page.getByRole('dialog', { name: '상한 초과 저장 확인', exact: true })).toBeHidden()
    ...
    await page.goto('/budgets?month=2026-04')
    await expect(page).toHaveURL('/budgets?month=2026-04')
    const aprilBudget = page.getByLabel('식비 예산')
    const aprilFood = page.getByRole('article').filter({ has: page.getByRole('heading', { name: '식비', exact: true }) })
    await aprilFood.getByRole('button', { name: /^지난달 예산 / }).click()
    await expect(aprilBudget).toHaveValue('500000')
    await aprilBudget.fill('460000')
    ...
    await expect(aprilBudget).toHaveValue('460000')
    ...
    await page.screenshot({ fullPage: true, path: testInfo.outputPath('budget-editor-390-dark.png') })
    await page.setViewportSize({ width: 1280, height: 720 })
    await aprilBudget.fill('500000')
    await page.getByRole('button', { name: '변경사항 저장', exact: true }).click()
    await expect(page.getByRole('button', { name: '저장됨', exact: true })).toBeVisible()
    await page.reload()
    await expect(aprilFood.getByRole('button', { name: /^지난달 예산 / })).toHaveAttribute('aria-pressed', 'true')
    ...
    const postingDialog = page.getByRole('dialog', { name: '2026-04 정기거래 선택 반영', exact: true })
    await postingDialog.getByRole('checkbox', { name: 'E2E 정기비용', exact: true }).check()
    ...
    await expect(page).toHaveURL('/ledger?month=2026-04&tab=list')

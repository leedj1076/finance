...
test('family user can manage a transaction and change their password', async ({ page }) => {
  ...
  try {
    ...
    const flowTrendChart = page.getByRole('img', { name: '월별 지출 추이' })
    await flowTrendChart.locator('[aria-label="1월 지출 3,000,000원"]').hover()
    await expect(page.getByRole('tooltip')).toContainText('지출: 3,000,000원')
    ...
    const netWorthChart = page.getByRole('img', { name: '월별 순자산 추이' })
    await netWorthChart.locator('[aria-label="2월 자산 요약"]').hover()
    await expect(page.getByRole('tooltip')).toContainText('순자산: 850,000원')
    await expect(page.getByRole('tooltip')).toContainText('총자산: 1,200,000원')
    await expect(page.getByRole('tooltip')).toContainText('부채: 350,000원')

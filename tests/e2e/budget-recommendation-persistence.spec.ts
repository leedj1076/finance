...
async function enqueueAndComplete(page: import('@playwright/test').Page, fixture: BudgetAcceptanceFixture) {
  ...
  if (!await dialog.isVisible()) await page.getByRole('button', { name: 'AI 추천 받기', exact: true }).click()
  ...
  await expect(page.getByRole('button', { name: '다시 추천', exact: true })).toBeVisible({ timeout: 15_000 })
  return claimed!
}
...
async function navigateToClientBudgetPanel(
  ...
  return page.getByRole('form', { name: '예산 편집기', exact: true })
}
...
async function waitForCompletedBudgetPanel(
  ...
  await expect(panel.getByRole('button', { name: '다시 추천', exact: true })).toBeVisible({ timeout: 15_000 })
}
...
suite('keeps manual saving available while a fixture worker is unsupported, absent, or offline', async ({ page, budgetFixture }) => {
  ...
  await expect(page.getByRole('button', { name: 'AI 추천 받기', exact: true })).toBeDisabled()
  ...
  await expect(absentPanel.getByText(
    'Mac AI 작업기가 연결되지 않았습니다', { exact: true },
  )).toBeVisible()
  await expect(page.getByRole('button', { name: 'AI 추천 받기', exact: true })).toBeDisabled()
  ...
  await expect(offlinePanel.getByRole('button', { name: 'AI 추천 받기', exact: true })).toBeEnabled()
...
suite('persists a real completed recommendation with its adjusted amount and frozen reasons', async ({ page, budgetFixture }, testInfo) => {
  ...
  await expect(page.getByRole('form', { name: '예산 편집기', exact: true })).toBeVisible()
  await page.getByRole('button', { name: 'AI 추천 받기', exact: true }).click()
  expect(fixture.nextMonth).toBe(shiftMonth(fixture.month, 1))

  await page.getByLabel('참고 메모', { exact: true }).fill('가족 식사를 우선해 주세요.')
  ...
  await expect(page.getByRole('form', { name: '예산 편집기', exact: true })).toHaveCount(1)
  await expect(page.getByRole('region', { name: '예산 편집 목록', exact: true })).toHaveCount(1)
  const reasonButton = page.getByRole('article').filter({ has: page.getByRole('heading', { name: '식비', exact: true }) }).getByRole('button', { name: '더 보기', exact: true })
  const reasonDetails = page.getByRole('dialog', { name: '식비 AI 추천 근거', exact: true })
  await reasonButton.click()
  await expect(reasonDetails).toBeVisible()
  ...
  await page.getByRole('dialog', { name: 'AI 예산 추천 요청', exact: true }).getByRole('button', { name: '추천 요청', exact: true }).click()
...
suite('rejects stale-window and stale-source AI saves until the user converts the draft to manual', async ({ browser, page, budgetFixture }, testInfo) => {
  ...
  try {
    ...
    await page.goto(`/budgets?month=${fixture.month}`)
    const job = await enqueueAndComplete(page, fixture)
    await page.getByRole('article').filter({ has: page.getByRole('heading', { name: '식비', exact: true }) }).getByRole('button', { name: /^AI 추천 / }).click()
    ...
    `
    await other.reload()
    await expect(other.getByText('기록이 바뀌어 다시 추천이 필요합니다', { exact: true })).toBeVisible()
    ...
    // Typing keeps AI provenance; a positive historical choice clears it.
    await other.getByRole('article').filter({ has: other.getByRole('heading', { name: '식비', exact: true }) }).getByRole('button', { name: /^지난달 실적 / }).click()
    await other.getByLabel('식비 예산', { exact: true }).fill('330000')
    await other.getByRole('button', { name: '변경사항 저장', exact: true }).click()
    await expect(other.getByRole('button', { name: '저장됨', exact: true })).toBeVisible({ timeout: 15_000 })
    ...
    await expect(other.getByText('직접 입력', { exact: true })).toBeVisible()

...
export async function buildLedgerCategoriesBrowser() {
  const root = process.cwd()
  const require = createRequire(path.join(root, 'package.json'))
  const bundled = require('next/dist/compiled/webpack/webpack')
  bundled.init()
  const webpack = bundled.webpack
  const output = await mkdtemp(path.join(tmpdir(), 'ledger-categories-browser-'))
  const compiler = webpack({
    mode: 'development', devtool: false,
    entry: path.join(root, 'tests/e2e/fixtures/ledger-categories-browser.tsx'),

...
| `stats.html` | **통계(연간) 화면 확정안** — 올해 성적 · 수입·지출·저축 · 달마다(선택형 그래프 + 표, 같은 12개 열) · 어디에 썼나 · 전년 비교/예측 |
| `stats-demo.dc.html` | 통계 ②③ 블록의 **동작 데모 소스** — 단독 실행 불가(캔버스 런타임 필요), 로직 참고용. 실제 동작은 캔버스 "통계 개편" 페이지에서 |

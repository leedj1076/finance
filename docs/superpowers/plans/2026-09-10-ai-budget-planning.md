...
- Tests: `tests/finance/budget-recommendation-panel.test.tsx`, `tests/finance/budget-recommendation-panel-controls.test.tsx`, `tests/finance/budget-draft.test.ts`, `tests/finance/budget-planning.test.ts`, `tests/finance/month-status-pages.test.tsx`, `tests/integration/budget-recommendation-service.test.ts`, `tests/e2e/budget-recommendations.spec.ts`
...
- Add the corresponding `readSavedBudgetRecommendations(reader: BudgetReader, householdId: string, month: string)` so planning data loads saved reasons inside its existing repeatable-read transaction. Pass `savedRecommendations` and canonical `basis` through the explicit page/Form props; update the two existing planning/page fixtures while preserving their canonical ceiling, month and single-editor assertions. Manual editing does not depend on an AI evidence snapshot or worker availability.
- Cover this new query in the existing scoped service integration suite: historical referenced jobs remain available after a newer recommendation/source change; invalid referenced reports are skipped without deleting budgets; other household/month jobs are never returned. Use the existing synthetic queue fixtures and normal RPC completion paths, not mutation of immutable jobs.
...
- `RecommendationRowContext = { jobId: string; recommendation: BudgetRecommendationReport['rows'][number]; snapshot: Pick<BudgetRecommendationSnapshot, 'month' | 'input' | 'evidence' | 'recurring'>; promptInput: AiPromptInput | null }` pairs a validated report row with its exact frozen reference context. Define it alongside the row component.
- `BudgetRow({ row, baseline, source, actual, month, period, recommendation, origin, selected, onSelect, onEdit, onManual }: { row: BudgetDraftRow; baseline: BudgetBaseline; source: BudgetSourceRow | null; actual: number; month: string; period: 'past' | 'current' | 'future'; recommendation: RecommendationRowContext | null; origin: RecommendationRowContext | null; selected: boolean; onSelect: (selected: boolean) => void; onEdit: (amount: string) => void; onManual: () => void })` renders amount/origin/reason accessibly. Pass `actual` from the canonical planning row so current actual and remaining stay visible before any AI report; nullable `source` is only AI snapshot comparison data and must not be fabricated or treated as zero. Resolve origin by the draft row's recommendationJobId and major in the validated saved/current report map; the latest recommendation may differ from that origin. Use each context's own snapshot/promptInput for evidence, never current settings or another report. Never accept browser-provided reason text for persistence.
- Add reducer action `{ type: 'rebase'; rows: BudgetBaseline[] }` for explicit conflict refresh/compare only. Determine dirty rows against the previous baseline using amount OR provenance. Retain only those user-edited rows; previously clean rows adopt fresh server amount/provenance, new majors initialize from fresh baseline, removed majors disappear, selection is filtered, and undo is cleared. The fresh baseline/CAS tokens are adopted while the user reviews current versus draft before retry; do not silently rebase on every incoming prop change or turn untouched old values into new writes. Preserve the target draft only if explicitly edited. Cover these cases in reducer tests and conflict E2E.
...
- Update existing E2E consumers of changed contracts: `tests/e2e/auth.spec.ts` (accessible single-editor identity and explicit overage consent), `tests/e2e/period-sorting-charts.spec.ts` (explicit overage consent while retaining integer validation), `tests/e2e/diagnosis.spec.ts` (real capability heartbeat and configured claim for new frozen-prompt requests, retaining legacy claim isolation coverage).
...
Integration checkpoint clarification: the first full E2E run exposed obsolete input-name selectors, missing explicit overage consent, and legacy-only worker fixtures in these existing browser tests. Update those tests to the already approved accessible editor/save/prompt contracts rather than weakening production controls. Preserve actual DB persistence assertions, whole-month diagnosis report/evidence/stale assertions, and prove a legacy claim cannot consume the newly configured job before a supported claim succeeds. Locally observed save-response waits may use a bounded 15-second assertion timeout under the unchanged full-suite concurrency; do not add sleeps, global retries, or suppress failures. All six gates must then pass freshly on the resulting tree.

Measured save-reliability follow-up: a bounded instrumented local acceptance run distinguished four PostgreSQL `40001` serialization aborts from actual row-CAS rejection. Before final gates, add a separately committed and reviewed fix in `src/features/budgets/save-service.ts` with focused regression coverage in `tests/integration/budget-recommendation-save.test.ts`: retry only `40001`, at most three whole SERIALIZABLE transaction attempts with the original parsed request. Every attempt reacquires the existing locks, rereads state, and repeats original CAS, source/provenance, household, target and overage checks. Never rebase expected versions, retry explicit application conflicts or `23505`/`40P01`, or add generic transaction infrastructure. Test rollback-before-retry, exhaustion, and real row/source changes between attempts; retain existing negative tests. Commit only these source/test paths as `fix: retry serializable budget saves safely`. This does not resolve the separately observed post-result UI pending state; that requires its own evidence-backed lifecycle design and review, not a timeout increase or forced enable.

Measured UI follow-up: the same retained trace proves one save committed, its action result returned, the marked header completed and response finished within 95ms, but the budget form's generic transition pending remained true past15s. Use a separately reviewed request-owned `onSubmit` lifecycle in `src/features/budgets/budget-form.tsx`: snapshot FormData before disabling, synchronously guard duplicate submissions with a ref, retain mounted-month/request ownership, call the unchanged server action, apply only the matching authoritative result, and release only its own terminal lock. Button and fieldset share this pending state; preserve native validation, error drafts, comparison/apply guards, frozen recommendation contexts, undo, latest-target and late-response protections. Do not add timers, after(), refresh workarounds, API routes, framework upgrades, or claim a proven upstream cause. Add focused real behavior regressions for unresolved vs acknowledged requests, independent transition work, duplicate submit, errors and unmount/month ownership; retain Task12/13 browser cases. Commit this bounded client/test follow-up separately as `fix: tie budget save feedback to request completion`, then integrate reviewed backend fix before final gates.
- [ ] **5. Commit.**
...
git add tests/e2e/auth.spec.ts tests/e2e/period-sorting-charts.spec.ts tests/e2e/diagnosis.spec.ts docs/superpowers/plans/2026-09-10-ai-budget-planning.md

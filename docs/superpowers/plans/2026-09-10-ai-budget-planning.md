...
- `BudgetRow({ row, baseline, source, month, period, recommendation, origin, selected, onSelect, onEdit, onManual }: { row: BudgetDraftRow; baseline: BudgetBaseline; source: BudgetSourceRow | null; month: string; period: 'past' | 'current' | 'future'; recommendation: RecommendationRowContext | null; origin: RecommendationRowContext | null; selected: boolean; onSelect: (selected: boolean) => void; onEdit: (amount: string) => void; onManual: () => void })` renders amount/origin/reason accessibly. Resolve origin by the draft row's recommendationJobId and major in the validated saved/current report map; the latest recommendation may differ from that origin. Use each context's own snapshot/promptInput for evidence, never current settings or another report. Never accept browser-provided reason text for persistence.
